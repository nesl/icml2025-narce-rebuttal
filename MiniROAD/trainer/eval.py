...
from utils import thumos_postprocessing, perframe_average_precision, perframe_average_F1
...
class Evaluate(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.eval_method = perframe_average_F1
    ...
    def eval(self, model, dataloader, logger):
        ...
        with torch.no_grad():
            ...
            for rgb_input, target in tqdm(dataloader, desc='Evaluation:', leave=False):
                rgb_input, flow_input, target = rgb_input.to(device), rgb_input.to(device), target.to(device)
