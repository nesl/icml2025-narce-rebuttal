...
class OadLoss(nn.Module):
    ...
    def __init__(self, cfg, reduction='mean'):
        ...
        self.criterion = focal_loss(alpha=torch.tensor([.005, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25]),gamma=2, device='cuda')
    ...
    def end_loss(self, out_dict, target):
        logits = out_dict['logits']  # (B, seq, C)
        target = target  # (B, seq)

        # Flatten
        logits = logits.view(-1, logits.size(-1))  # (B * seq, C)
        target = target.view(-1)                   # (B * seq,)
